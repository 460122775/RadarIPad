...
#define WINDOW_COLOR                            [UIColor colorWithRed:0 green:0 blue:0 alpha:0]
#define ACTIONSHEET_BACKGROUNDCOLOR             [UIColor colorWithRed:247/255.00f green:247/255.00f blue:247/255.00f alpha:0.7]
...
#define SHARETITLE_INTERVAL_WIDTH               34
...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray;
{
    ...
    if (self) {
        //初始化背景视图，添加手势
        self.frame = CGRectMake(342, 224, 320, 300);
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                if (self.isHadTitle == YES) {
                    [shareLabel setFrame:
                     CGRectMake(SHARETITLE_INTERVAL_WIDTH+((line-1)*(SHARETITLE_INTERVAL_WIDTH+SHARETITLE_WIDTH)),
                                                    self.LXActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
                }
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        ...
        [self.backGroundView setFrame:CGRectMake(0, self.frame.size.height -self.LXActivityHeight, self.frame.size.width, self.LXActivityHeight)];
